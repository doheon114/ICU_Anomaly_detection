...
input_dir = '/home/work/.doheon114/ICU_Anomaly/pseudo_labels_one/images'
output_dir = '/home/work/.doheon114/ICU_Anomaly/pseudo_labels_one/images_depth'
output_dir_aug = '/home/work/.doheon114/ICU_Anomaly/pseudo_labels_one/images_aug'
...
for file_path in png_files:
    ...
    if img is None:
        ...
        continue
    d = RGB2D(img)
    # 가우시안 노이즈 추가 (평균 0, 표준편차 0.01)
    noise = np.random.normal(0, 0.01, d.shape)
    d = d + noise
    # input_dir 이후의 상대 경로 추출
    ...
    np.save(os.path.join(save_dir, f"{name}.npy"), d)
    print(f"{file_path} -> {os.path.join(save_dir, f'{name}.npy')} 저장 완료")

    # # D2RGB로 변환 후 images_aug에 PNG로 저장
    # save_dir_aug = os.path.join(output_dir_aug, rel_dir)
    # os.makedirs(save_dir_aug, exist_ok=True)
    # rgb_img = D2RGB(d)
    # cv.imwrite(os.path.join(save_dir_aug, f"{name}.png"), rgb_img)
    # print(f"{file_path} -> {os.path.join(save_dir_aug, f'{name}.png')} 변환 및 저장 완료")
